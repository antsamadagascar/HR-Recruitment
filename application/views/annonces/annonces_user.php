...
<body><?php if (isset($annonces) && !empty($annonces)): ?>
    <form action="CV_Controller" method="post">
        <!-- Champ caché pour l'ID de l'annonce -->
        <input type="hidden" name="idAnnonce" value="<?php echo $annonces->idannonce; ?>" />
...
<?php else: ?>
    <div class="no-annonces-message">
        <p>Aucune annonce disponible pour le moment. Veuillez revenir plus tard.</p>
    </div>
<?php endif; ?>

<script>
    function postulerCV(annoncesId) {
        alert('Redirection vers le formulaire de candidature pour l\'annonce ' + annoncesId);
    }
</script>
